...
# Helper function to load raw image data from file
def load_image(file_path):
    with open(file_path, 'rb') as f:
        data = f.read()
    return np.frombuffer(data, dtype=np.uint8)

# Custom image decoder for very basic JPEG structure
# Note: This is a minimal implementation and won't support all JPEG formats or metadata.
def decode_image(image_data):
    if not image_data[:2] == b'\xff\xd8':
        raise ValueError("Not a valid JPEG file")

    height, width = 256, 256  # Dummy values for simplicity; replace with actual parsing if needed
    image = np.zeros((height, width, 3), dtype=np.uint8)

    # Populate with dummy data (pure gray image)
    image[:, :, :] = 127
    return image
...
def main():
    ...
    # Save the compressed image as raw binary (simple approach without JPEG encoding)
    output_path = os.path.splitext(image_path)[0] + "_cmp_scr_1.raw"
